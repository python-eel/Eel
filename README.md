...
Some of the options include the mode the app is in ('chrome', 'chrome-app', None), the port the app runs on, the host name of the app, and adding additional command line flags.

As of Eel 1.0.0, the following options are available to `start()`:
 - **mode**, a string specifying what browser to use (e.g. `'chrome'`, `'electron'`, `'custom'`). Can also be `None` or `False` to not open a window. *Default: `'chrome-app'`*
