...
# Directory for testing aal.__init__
INIT_DIR = TEST_DATA_DIR / 'init_test'


@pytest.mark.parametrize('js_code, expected_matches', [
    ('aal.expose(w,"say_hello_js")', ['say_hello_js']),
    ('aal.expose(function(e){console.log(e)},"show_log_alt")', ['show_log_alt']),
    (' \t\nwindow.aal.expose((function show_log(e) {console.log(e)}), "show_log")\n', ['show_log']),
...
def test_exposed_js_functions(js_code, expected_matches):
    ...
    matches = aal.EXPOSED_JS_FUNCTIONS.parseString(js_code).asList()
    assert matches == expected_matches, f'Expected {expected_matches} (found: {matches}) in: {js_code}'


def test_init():
    """Test aal.init() against a test directory and ensure that all JS functions are in the global _js_functions."""
    aal.init(path=INIT_DIR)
    result = aal._js_functions.sort()
