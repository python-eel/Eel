...
aal.set_host('ws://localhost:8080')

// Expose the `sayHelloJS` function to Python as `say_hello_js`
function sayHelloJS(x: any) {
  console.log('Hello from ' + x)
}
// WARN: must use window.eel to keep parse-able aal.expose{...}
window.aal.expose(sayHelloJS, 'say_hello_js')

// Test anonymous function when minimized. See https://github.com/samuelhwilliams/Eel/issues/363
function show_log(msg: string) {
  console.log(msg)
}
window.aal.expose(show_log, 'show_log')

// Test calling sayHelloJS, then call the corresponding Python function
sayHelloJS('Javascript World!')
aal.say_hello_py('Javascript World!')
...
export class App extends Component<{}, {}> {
  ...
  public pickFile = () => {
    aal.pick_file(defPath)((message: string) => this.setState({ message }))
  }

  public render() {
    aal.expand_user(defPath)((path: string) => this.setState({ path }))
    return (
      <div className="App">
